...
class DataPreprocessing:
    """
    Data preprocessing methods that we tried but did *not* yield good results
    """
    ...
    def view_buy_convert(x, x2):
        ...
        :param x2: key into dictionary to access the array
        :return: length of array
        """
        ttx = literal_eval(x)

        if ttx.get(x2) is not None:
            return len(ttx.get(x2))
        return 0
