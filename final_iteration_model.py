...
from first_iteration_model import FirstIterationModel
import xgboost as xgb


class FinalIterationModel:
    """
    Our final model for Deliverable #4. We predict using reviewText, summary, review count, root-genre, and related.
    We perform relevant preprocessing on these features. This file contains code for model selection, hyperparameter
    tuning of XGBoost, and all preprocessing required.
    """
    ...
    def preprocess(df, test_file=False):
        """
        Performs all preprocessing necessary for this third iteration of our model; only columns we need are reviewText,
        summary, root-genre, and related. We do all processing and engineering here.
        :param df: DataFrame to be processed
        :param test_file: boolean indicating whether this is Test.csv DataFrame (processed a little differently because
                            it does not have overall column)
        ...
        df['reviewText'] = df['reviewText'] + ' ' + ((df['summary'] + ' ') * 5)  # weigh summary more
        ...
        # get target col (awesomeness) for test file only
        if not test_file:
            df_target = df.groupby('amazon-id')['overall'].mean() > 4.5
            final_df = pd.merge(chosen_cols, df_target, on='amazon-id')
            final_df = pd.merge(final_df, df_concat_text, on='amazon-id')
        else:
        ...
        # rename col from overall to target
        if not test_file:
            final_df.rename(columns={'overall': 'target'}, inplace=True)
    ...
    def evaluate_xgb(x_train, y_train, x_test, y_test, model_name, estimators):
        ...
        have used the other evaluate method from FirstIterationModel with) but Learning API is significantly faster
    ...
    def get_predictions_from_train(df, num_split, model_selection=False, tune_lr=False, model='xgb', estimators=-1):
        ...
        :param model: string indicating whether you want to try Logistic Regression ('lr'), AdaBoostClassifier ('ada')
        ...
        for train_index, test_index in cv.split(df):
            ...
            x_train = df_train[['reviewText', 'review_compound', 'sum_compound', 'view_buy', 'root-genre', 'review-count']]
            y_train = df_train['target']
            x_test = df_test[['reviewText', 'review_compound', 'sum_compound', 'view_buy', 'root-genre', 'review-count']]
            y_test = df_test['target']

            tfidf = TfidfVectorizer(analyzer='word', max_features=10000, ngram_range=(1, 2), token_pattern=r'\w{1,}')

            x_train_text, x_test_text = tfidf.fit_transform(x_train['reviewText']), tfidf.transform(x_test['reviewText'])
            ...
            if model_selection:
                # every classification model on Canvas homepage, except SVM because that takes a very long time
                FirstIterationModel.evaluate_model(MultinomialNB(), x_train_full, y_train, x_test_full, y_test,
                                                   "multinomialnb")
                FirstIterationModel.evaluate_model(RandomForestClassifier(), x_train_full, y_train, x_test_full, y_test,
                                                   "randomforest")
                FirstIterationModel.evaluate_model(DecisionTreeClassifier(), x_train_full, y_train, x_test_full, y_test,
                                                   "decisiontree")
                FirstIterationModel.evaluate_model(KNeighborsClassifier(), x_train_full, y_train, x_test_full, y_test,
                                                   "kneighbors")
                FirstIterationModel.evaluate_model(GaussianNB(), x_train_full, y_train, x_test_full, y_test,
                                                   "gaussiannb")
                FirstIterationModel.evaluate_model(LogisticRegression(), x_train_full, y_train, x_test_full, y_test,
                                                   "logisticregression")
            ...
            if tune_lr:
                for solver in ['newton-cg', 'lbfgs', 'liblinear', 'sag', 'saga']:
                    for penalty in ['none', 'l1', 'l2', 'elasticnet']:
                        try:
                            FirstIterationModel.evaluate_model(LogisticRegression(solver=solver, penalty=penalty),
            ...
            if model == 'lr' and clf is not None:
                f1 = FirstIterationModel.evaluate_model(clf, x_train_full, y_train, x_test_full, y_test, model)
    ...
    def tune_booster(df, booster='xgb', folds=10):
        ...
        MORE parameters (reference the Excel sheet!). However, doing a grid search on these multiple parameters
        gave many memory errors and so basically never ended. Instead, we ran many times, changing different parameters
        one at a time, and deduced the best ones that way. Please consider this as an example of the kind of tuning
        that we performed.
        :param df: full DataFrame
    ...
    def train_and_predict(train, test):
        ...
        y_pred = clf.predict(d_test)
        y_pred = np.round(y_pred).astype(int)  # convert to int for grading (not actually necessary)
        output = pd.DataFrame({'amazon-id': test['amazon-id'], 'Awesome': y_pred})
        output.to_csv('product_predictions_final.csv')
