...
class FirstIterationModel:
    ...
    We split the Train.csv data set into a train and test set (instance variables below) in order to assess the impacts
    ...
    def __init__(self, csv):
        ...
        :param csv: file path of Train.csv, or a preprocessed variant of this file
        """
        df = pd.read_csv(csv)
    ...
    def train_and_predict(self, model_selection=False):
        ...
        if model_selection:
            FirstIterationModel.evaluate_model(GaussianNB(), x_train, y_train, x_test, y_test, "gaussian nb")
            FirstIterationModel.evaluate_model(DecisionTreeClassifier(), x_train, y_train, x_test, y_test, "decision tree")
            FirstIterationModel.evaluate_model(KNeighborsClassifier, x_train, y_train, x_test, y_test, "k neighbors")
            FirstIterationModel.evaluate_model(LogisticRegression(), x_train, y_train, x_test, y_test, "logistic regression")
            FirstIterationModel.evaluate_model(RandomForestClassifier(), x_train, y_train, x_test, y_test, "random forest")
        ...
        # 0.21 corrector improves score (Joseph's discovery)
